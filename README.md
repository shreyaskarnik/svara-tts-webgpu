...
> **Status: starting-point scaffold.** The LM + SNAC ONNX inference pipeline is wired end-to-end (no streaming yet — full generation, then SNAC decode in one shot). Voice selector covers all 38 Svara voices. First-load is ~2.65 GB (cached after).
...
| Repo | Size | Notes |
|------|------|-------|
| [`shreyask/svara-tts-v1-ONNX`](https://huggingface.co/shreyask/svara-tts-v1-ONNX) | ~2.6 GB | Llama-3.2-3B q4f16, GQA, KV-cache |
...
First run downloads ~2.65 GB into the browser cache (LM + codec + tokenizer). Subsequent runs are instant.
...
- Service worker for offline caching of the 2.6 GB model.
